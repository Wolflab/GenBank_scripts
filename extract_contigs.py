...
output file (output.fasta) is just those you want to extract. Output file name shoule be 
third argument, but will give default name if not specified
...
def extract_contigs(fasta_file, contigs_to_extract_list, out_name):
    out = open(out_name, 'w')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        outfile_name = 'extracted_output.fasta'
    else:
        outfile_name = sys.argv[3]
    extract_contigs(fasta_file,contigs_to_extract_list, outfile_name)
