...
if(identical(missingAccessions, character(0))){
    ...
    send.mail(from ="yahboiyahweh@yahoo.com", 
            to = c("paul.wolf@usu.edu", "robison.tanner@gmail.com"), 
            subject = "Fern Plastome Query", 
            body= "No new plastomes at this time", 
            smtp= list(host.name = "smtp.mail.yahoo.com", port = 465, user.name = "yahboiyahweh@yahoo.com", passwd = "FernPlastomes", ssl = TRUE),
            authenticate = TRUE, 
            send =TRUE)
}else{
