...
if(os.path.exists(os.getcwd()+'/checked_accession_numbers.csv')):
    oldListFile = open('checked_accession_numbers.csv');
    for line in oldListFile:
        oldList.append(line.strip())
    oldListFile.close();
else:
    open('checked_accession_numbers.csv', 'w').close()

if not os.path.exists(os.getcwd()+'/accession_numbers.csv'):
    open('accession_numbers.csv', 'w').close()
    ...
    if match[1] not in oldList:
        ...
        if idMatch:
            ...
            if docContent.find(keyword) > -1:
                outputFile = open('accession_numbers.csv', 'a')
                outputFile.write(match[1]+'\n')
                outputFile.close()
        else:
            if(os.path.exists(os.getcwd()+'/checked_accession_numbers.csv')):
                manualCheckFile = open('manual_check.csv', 'a')
                manualCheckFile.write(match[1]+'\n')
                manualCheckFile.close()
            else:
                manualCheckFile = open('manual_check.csv', 'w')
                manualCheckFile.write(match[1]+'\n')
                manualCheckFile.close()
        #fetch the article thing and search for keyword
        oldListFile = open('checked_accession_numbers.csv', 'a')
